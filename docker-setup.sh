...
# This file is used for one-click automatic download of project required files and automatic directory creation
# Currently only supports X86 version Ubuntu system, other systems not tested
...
if [ $EUID -ne 0 ]; then
    whiptail --title "Permission Error" --msgbox "Please run this script with root permissions" 10 50
    exit 1
fi
...
if check_installed; then
    if whiptail --title "Installation Detected" --yesno "Xiaozhi server installation detected, perform upgrade?" 10 60; then
        ...
        # Download latest version configuration files
...
if ! command -v docker &> /dev/null; then
    ...
    echo "Warning: Some key additions failed, continuing installation attempt..."
...
MIRROR_CHOICE=$(whiptail --title "Select Docker Mirror Source" --menu "Please select Docker mirror source to use" 20 60 10 \
...
case $MIRROR_CHOICE in
    ...
    7) MIRROR_URL=$(whiptail --title "Custom Mirror Source" --inputbox "Please enter complete mirror source URL:" 10 60 3>&1 1>&2 2>&3) ;; 
    8) MIRROR_URL="" ;; 
esac
...
# If not upgrade completed, then execute download
...
if [ $? -ne 0 ]; then
    whiptail --title "Error" --msgbox "Docker service startup failed, please try changing mirror source and re-execute this script" 10 60
    exit 1
fi
...
# Get server public network address
PUBLIC_IP=$(hostname -I | awk '{print $1}')
whiptail --title "Configure Server Secret Key" --msgbox "Please use a browser to visit the following link, open the control console and register an account: \n\nInternal network address: http://127.0.0.1:8002/\nPublic network address: http://$PUBLIC_IP:8002/ (if using cloud server, please open ports 8000 8001 8002 in server security group).\n\nThe first registered user is the super administrator, subsequent registered users are regular users. Regular users can only bind devices and configure agents; super administrators can perform model management, user management, parameter configuration, etc.\n\nAfter registration, press Enter to continue" 18 70
SECRET_KEY=$(whiptail --title "Configure Server Secret Key" --inputbox "Please log in to the control console using super administrator account\nInternal network address: http://127.0.0.1:8002/\nPublic network address: http://$PUBLIC_IP:8002/\nIn the top menu Parameter Dictionary → Parameter Management find parameter code: server.secret (server secret key) \nCopy this parameter value and enter it in the input box below\n\nPlease enter secret key (leave blank to skip configuration):" 15 60 3>&1 1>&2 2>&3)
...
# Fix issue where log file cannot get ws, changed to hardcoding
